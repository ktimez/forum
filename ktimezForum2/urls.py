"""ktimezForum2 URL Configuration
...
urlpatterns = [
    ...
    url(r'^category/(?P<pk>\d+)/$', views.category_topics, name='category_topics'),
    url(r'^category/(?P<pk>\d+)/new/$', views.new_topic, name='new_topic'),
    url(r'^signup/$', accounts_views.signup, name='signup'),
    url(r'^logout/$', auth_views.LogoutView.as_view(), name='logout'),
    url(r'^login/$', auth_views.LoginView.as_view(template_name='login.html'), name='login'),
    url(r'^category/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/$', views.topic_posts, name='topic_posts'),
    url(r'^category/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/reply/$', views.reply_topic, name='reply_topic'),
    url(r'^category/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/reply/(?P<post_pk>\d+)/edit/$', views.PostUpdateView.as_view(), name='edit_post'),
